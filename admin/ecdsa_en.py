...
##  encrypt with reading pk from pkforall.txt
with open('pkforall.txt', 'rb') as f:
    vk_byte= f.read()
    f.close()

msg = 'nihafdjsakfhsafdhsafhdsfdsahfjdashfkdashfhdashfdashfdashfdashfkjldashfjkdshfadshfo !!'
encrypt_result = encrypt(msg, vk_byte)
print("type of encrypt_result", type(encrypt_result))


##  decrypt with reading sk from pwd_user.txt
...
msg_re = decrypt(encrypt_result, pwd)



# pk = ecdsa.VerifyingKey.from_string(vk_string, curve=ecdsa.SECP256k1)
# max_len = ecdsa.SECP256k1.baselen
# msgs_ened_byte = b''
#
# G = pk.pubkey.generator
...
# r = k * G
#
# msgs_ened_byte += base64.b64encode(r.x().__int__().to_bytes(max_len,"big"))
#
# for i in deal_text(msg, max_len):
#     r_byte, msg_ened_int = ecc_en(pk, i, k, r)
#     assert msg_ened_int > 0
#     msg_ened_byte = msg_ened_int.__int__().to_bytes(max_len, "big")
#     msgs_ened_byte += msg_ened_byte
# encrypt_result = base64.b64encode(msgs_ened_byte)
...
# Q = vk.pubkey
# msg_int = 123
# ened = (msg_int + (k * Q.point).x())%n
# print(ened)
#
# #ened = 8640128784996030638003827482510526703994734568154357667283532689018127828170
#
# msg_re_int  = ened - (sk.privkey.secret_multiplier * p1).x()
# print(msg_int, msg_re_int)
