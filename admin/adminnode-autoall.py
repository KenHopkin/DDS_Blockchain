...
if __name__ == "__main__":
    ...
    device[-1].node_idx = dev_id.index(args.master)

    for nodeid in nodeIDs:
        device.append(node_module.Node(Web3(Web3.HTTPProvider(dev_ip)), authensc, nodeid, w3.eth.accounts[dev_id.index(nodeid)]))
        device[-1].node_idx = dev_id.index(nodeid)

    device[0].mas_rst_contract2(w3.eth.accounts[dev_id.index(args.master)])
    ...
    print(device[0].mas_build_domain(masterid, task_id))
