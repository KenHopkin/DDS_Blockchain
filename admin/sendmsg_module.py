...
#args = argparse.Namespace(group= 'Task01', node = 'ship13', msg="nihao! !")
...
def send_message(group, node, recv, msg, flag):
    # 用HTTP连接三个节点
    ...
    device.append(
        node_module.Node(Web3(Web3.HTTPProvider(dev_ip)), authensc, node,
                         w3.eth.accounts[dev_id.index(node)]))

    #---------------------------------------------------------------------
    # encrypto for someone ship
    message = msg
    if flag == 0:
        file_name = str(dev_id.index(recv)-1)
        file_path = "./public_key/" + file_name + ".txt"
        print('pk_filename:', file_name)
        ##  encrypt with reading pk from the folder named "public_key"
        with open(file_path, 'rb') as f:
            vk_byte = f.read()
            f.close()
        print(message)
        print(vk_byte)
        message = ecdsa_ende_module.encrypt(message, vk_byte)

    print(device[0].device_au(dev_id.index(node), group, recv, message))
    #--------------------------------------------------------------------
