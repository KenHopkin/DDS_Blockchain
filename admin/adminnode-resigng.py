...
if __name__ == "__main__":
    ...
    tx_hash = device[1].contract.functions.DetouchPhase(device[1].cert[index].domain_id, device[1].cert[index].device_id,
                                                       device[1].cert[index].device_addr, device[1].cert[index].sig, device_sig) \
        .transact({'from': w3.eth.accounts[dev_id.index(args.node)]})
    # self.w3.geth.miner.start(2)
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    # self.w3.geth.miner.stop()

    res_used, res_id, res_addr, _, _, _ = device[1].contract.functions.ViewDeviceByID(device[1].cert[index].domain_id,
                                                                                 device[1].cert[index].device_id).call()
    if not (res_used and res_id == device[1].cert[index].device_id and res_addr == device[1].cert[index].device_addr):
        # device[1].domain_ass.remove(device[1].cert[index].domain_id)
        print( "the device:" + res_id + " at " + device[1].cert[index].device_addr + " is resigned successfully!")
    else:
        try:
    ...
    time_resign_device2 = datetime.datetime.now()
    print("无人艇out信任域所需时间：", (time_resign_device2 - time_resign_device1), "秒")
