...
parser.add_argument("-msg",help="massage to send", default="")

#-------------------------------------------------------------------
parser.add_argument("-recv",help="who can recv", default="")
#-------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    msg = args.msg
    if args.recv != "":
        addr_recv = w3.eth.accounts[dev_id.index(args.recv)]
        len_msg = len(msg.encode('utf-8'))
        msg = base64.b64encode(bytes([i ^ j for i, j in zip(msg.encode('utf-8'), (
        ((addr_recv[2:]) * (int(len_msg / 40) + 1))[:len_msg]).encode('utf-8'))])).decode()

    print(device[0].device_au(dev_id.index(args.node), args.group, args.recv, msg))
    #--------------------------------------------------------------------
