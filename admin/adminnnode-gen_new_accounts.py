...
def show_all_state():
    for i in range(20):
        ...
        while device[i].dev_dm_can_iterate(curr_dm):
            print(curr_dm, device[i].id, device[i].query_lgtlttwh(curr_dm, device[i].id))
...
if __name__ == "__main__":
    ...
    for i in range(1050):
        w3.geth.personal.new_account(pwd_dev)
    w3.geth.personal.unlock_account(w3.eth.accounts[101], pwd_dev)
    w3.geth.personal.unlock_account(w3.eth.accounts[102], "123456")
    # show_all_state()
